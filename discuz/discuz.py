...
log_path = os.path.join(discuz_DIR, 'discuz.log')

parser = argparse.ArgumentParser(description='Discuz auto checkin')
...
for key, value in envs.items():
    if key.startswith('discuz_username_'):
        ...
        https_proxy = os.getenv('discuz_https_' + env_id)
        if password:
            if http_proxy:
                proxies['http'] = http_proxy
                if https_proxy:
...
class Checkin(object):
    ...
    def __init__(self, account, retry=1, strage='local'):
    ...
    def discuz_login(self):
        ...
        if L7DFW:
            cookies['L7DFW'] = L7DFW
        headers = {
            'Host': urlparse(self.forumurl).netloc,
            #'Cookie': 'wyp7_2132_sid=nchIRV; wyp7_2132_noticeTitle=1; wyp7_2132_saltkey=Z6Ww5k0k; wyp7_2132_lastvisit=1580549164; wyp7_2132_sendmail=1; wyp7_2132_lastact=1580552788%09member.php%09',
            'cache-control': 'max-age=0',
            'origin': self.forumurl,
    ...
    def discuz_print_user_info(self, user_info_parsed, text_prefix=''):
        info("{prefix}用户: {username}, 用户组: {usergroup}, {credit_name_1}: {credit_value_1}, {credit_name_2}: {credit_value_2}, 积分: {credit}".format(
            prefix = text_prefix,
            username = self.username,
            usergroup = user_info_parsed.group(1),
            credit_name_1 = user_info_parsed.group(2),
            credit_value_1 = user_info_parsed.group(3),
            credit_name_2 = user_info_parsed.group(4),
            credit_value_2 = user_info_parsed.group(5),
    ...
    def discuz_get_user_id(self):
        if self.user_id > 0:
            info('user id had got before.')
            return True

        pattern = re.compile(r'''discuz_uid = ['"](\d+)['"],''')
        user_id = pattern.search(self.user_info)
        if user_id:
            self.user_id = int(user_id.group(1))
            info('user id got: {}'.format(self.user_id))
            return True
