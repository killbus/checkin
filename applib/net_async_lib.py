...
class NetManager(object):
    ...
    def __init__(self, loop=None, event_notify=None):
...
#-#        conn = aiohttp.TCPConnector(resolver=resolver, limit=10, ttl_dns_cache=300)
        ...
        if self.loop:
            self.sess = aiohttp.ClientSession(connector=conn, headers=headers, loop=self.loop)
        else:
            self.sess = aiohttp.ClientSession(connector=conn, headers=headers)
        info('sess inited.')
    ...
    async def getData(self, url, *args, **kwargs):
        ...
        str_encoding = kwargs.pop('my_str_encoding', 'utf-8')
...
#-#                debug('url %s %s %s', url, pcformat(args), pcformat(kwargs))
                ...
                if fmt == 'str':
                    try:
                        ...
                        warn(f'ignore decode error from {url}')
